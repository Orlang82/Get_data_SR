...
# =============================================================================
# КОНСТАНТЫ И НАСТРОЙКИ - ВСЕ НАСТРОЙКИ В ОДНОМ МЕСТЕ
# =============================================================================
...
TARGET_SHEETS = 'Верифікація моделі ES'  # Куда вставлять диаграмму лист

# Заголовки для спидометров
...
# Границы цветовых зон (одинаковые для обеих диаграмм)
...
def create_single_speedometer(ax, current_value, title, x_offset=0):
    ...
    def value_to_angle(value):
        return 180 - 180 * (value - SCALE_MIN) / (SCALE_MAX - SCALE_MIN)
    
    # Рисуем цветовые зоны
    zones = [
        (RED1_MIN, RED1_MAX, RED_COLOR),
        (YELLOW1_MIN, YELLOW1_MAX, YELLOW_COLOR),
        (GREEN_MIN, GREEN_MAX, GREEN_COLOR),
        (YELLOW2_MIN, YELLOW2_MAX, YELLOW_COLOR),
        (RED2_MIN, RED2_MAX, RED_COLOR)
    ]
    ...
    for i in range(9):
        val = SCALE_MIN + (SCALE_MAX - SCALE_MIN) * i / 8
    ...
    # Стрелка
    clamped_value = np.clip(current_value, SCALE_MIN, SCALE_MAX)
    ...
    center_circle = patches.Circle((x_offset, 0), CENTER_CIRCLE_RADIUS, 
                                  facecolor='white', edgecolor='black', linewidth=2)
    ax.add_patch(center_circle)
    ax.text(x_offset, 0, f'{current_value:.2f}', ha='center', va='center', 
           fontsize=CENTER_FONT_SIZE, weight='bold')
...
def create_double_speedometer_plot(value_1d, value_10d):
    ...
    left_position = -SPEEDOMETER_SPACING / 2
    right_position = SPEEDOMETER_SPACING / 2
    
    # Создаем два спидометра
    create_single_speedometer(ax, value_1d, TITLE_1D, left_position)
    create_single_speedometer(ax, value_10d, TITLE_10D, right_position)
    
    # ОБРЕЗАЕМ ГРАНИЦЫ - убираем пустое пространство снизу
    ax.set_xlim(left_position - SIDE_MARGIN, right_position + SIDE_MARGIN)
    ax.set_ylim(BOTTOM_MARGIN, TOP_MARGIN)  # Используем параметры из констант
    ...
    fig.savefig(temp_path, dpi=DPI, bbox_inches='tight', pad_inches=0.1)  # Минимальные отступы
    plt.close(fig)
...
def insert_image_to_excel():
    ...
    wb = xw.Book.caller()
    value_1d = wb.names[SOURCE_CELL_1D].refers_to_range.value
    value_10d = wb.names[SOURCE_CELL_10D].refers_to_range.value
    
    # Создаем двойной спидометр и получаем путь к файлу
    img_path = create_double_speedometer_plot(value_1d, value_10d)
    
    # Удаляем старое изображение
    ws = wb.sheets[TARGET_SHEETS]
    for pic in ws.pictures:
        if pic.name == IMAGE_NAME:
            pic.delete()
    
    # Вставляем новое изображение
    pic = ws.pictures.add(img_path, name=IMAGE_NAME, update=True,
                         left=ws.range(TARGET_CELL).left,
                         top=ws.range(TARGET_CELL).top)
